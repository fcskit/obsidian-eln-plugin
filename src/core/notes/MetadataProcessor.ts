...
/**
 * Handles metadata template processing and transformation
 */
    ...
    applySubclassTemplate(
        ...
        if (subclassTemplate.replace && Array.isArray(subclassTemplate.replace)) {
            for (const { fullKey, newKey, input } of subclassTemplate.replace) {
                ...
                // Add new key
                if (newKey) {
                    obj[newKey] = input as unknown as MetaDataTemplateFieldProcessed;
                }
            }
        }
    ...
    async processMetadata(
            ...
            if (
                ...
                if (field.inputType === "number" && field.units) {
                    ...
                    if (inputValue && typeof inputValue === "object" && "value" in inputValue && "unit" in inputValue) {
                        ...
                        result[key] = { value: inputValue as number, unit } as FormFieldValue;
                    } else if (defaultValue !== undefined) {
                        // If no user input, use the default value and unit
                        result[key] = { value: defaultValue as number, unit } as FormFieldValue;
                    } else {
                        // If no default value, set to null
                        result[key] = { value: null, unit } as FormFieldValue;
                    }
                } else {
                    // Handle other input types
                    result[key] = inputValue !== undefined ? inputValue : (defaultValue as FormFieldValue);
                }
            }
        }
        ...
        for (const [key, value] of Object.entries(userInput)) {
            if (!(key in template) && value !== undefined) {
                console.debug(`Adding additional key from userInput: ${key}`, value);
                result[key] = value;
            }
        }
