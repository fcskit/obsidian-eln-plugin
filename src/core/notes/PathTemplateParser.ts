...
import { PathTemplate, FormData, FormFieldValue } from "../../types";
...
export async function parsePathTemplate(
    ...
    logger.debug("Date localization set to:", dateLocalization);
    ...
    logger.debug("Default lookup fields:", lookupFields);

    // Merge context into lookup fields
    if (context) {
        lookupFields = { ...lookupFields, ...context };
        logger.debug("Merged lookup fields with context:", lookupFields);
    }

    // Process the template
    let path = template
        .map(({ type, field, separator }: TemplateElement) => {
            logger.debug(`Processing template element:`, { type, field, separator });
            ...
            logger.debug(`Resolved value for field "${field}":`, value);
 ...
 */
function getFieldValue(field: string, context: Record<string, unknown>): string | undefined {
